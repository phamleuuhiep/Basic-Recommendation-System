...
def get_diverse_recommendations(cosine_similarity_df, user_movie, num_recommendations=5):
    data = cosine_similarity_df.loc[user_movie, :]
    diverse_movies = data.sort_values(ascending=True)[:num_recommendations]
    return diverse_movies
    
# Example usage
diverse_result = get_diverse_recommendations(cosine_similarity_df, user_input)
